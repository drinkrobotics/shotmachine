...
void move_right_until_endswitch_detected(){
  ...
  for (position=0; position<4000; position++){
...
void move_steps(int steps, int dir){
  ...
  for (position=0; position<steps; position++){
    if(eStopEnabled){
      terminal.println("E Stop enabled, press start to disable. This happened because end switch was pressed and it was already clibrated.");
      break;
    }
...
void make_shots(int number){
  ...
  if(!calibrated){
    terminal.println("Not calibrated returning");
    return;
  }
  ...
  int which_pump_array[number];

  if(digitalRead(pinModeSwitch) == LOW){
    //russian roulette mode: fill the glasses in random order

    //set all arry elements to pump 0
    for(int pos=0; pos < number; pos++){

      which_pump_array[pos]= 0;
    ...
    for(int bullet_number = 0; bullet_number < NUMBER_OF_BULLETS_IN_RUSSIAN_ROULETTE; bullet_number++){
        ...
        //find a space in the array with no bullet and not reserved as cheat_shot
        while(which_pump_array[random_number]==1 || random_number == cheat_shot){    
          SwSerial.print("Position not suitable for bullet because either there was already a bullet or its a cheat shot ");
        ...
        which_pump_array[random_number]=1;

        //set new cheat shot one place to left
        if(cheat_shot == number-1){
          cheat_shot = 0;
        }else{
          cheat_shot++;
        }
    ...
    for(int pos=0; pos < number; pos++){
      which_pump_array[pos]=pos%2;
    }
  }

  //actually fill in the shots
  for(int pos=0; pos < number; pos++){
    ...
    fill_glass(which_pump_array[pos]);
...
void fill_glass(int pump){
  ...
  BLYNK_WRITE(V1)
  {
    SwSerial.print("Got message V1 virtual start button");
  ...
  BLYNK_WRITE(V3)
  {
    SwSerial.print("Got message V3 Number of Shots");
